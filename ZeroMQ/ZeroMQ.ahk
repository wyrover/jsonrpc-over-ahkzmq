class ZeroMQ extends ZeroMQ_Constant
{
        ...
        loop
        {
            ...
            else
            {
                if (merge_frames)
                    ...
                    ;puts(StrGet(data_ptr, full_size, encoding))
    ...
    poll(ByRef poller_events, timeout := -1)
    {
        address := this.GetAddress("zmq_pollitem_t")

        rc := DllCall(this.zmq_poll
            , "Ptr", address
            , "Int", this.n_items
            , "Int", timeout
            , "Int") 
        ...
        loop % this.n_items
        {
            ...
            ;poller_events[A_Index] := revents & this.events[A_Index]
            poller_events[A_Index] := revents 
        }
    ...
    curve_keypair()
    {
        VarSetCapacity(public_key, 41)
        VarSetCapacity(secret_key , 41)

        rc := DllCall(this.zmq_curve_keypair, "Ptr", &public_key, "Ptr", &secret_key , "Int")

        return [StrGet(&public_key,, "utf-8") , StrGet(&secret_key,, "utf-8") ]
    }
}
